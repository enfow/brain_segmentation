...
class SegNet_CNN4A_FC6C(nn.Module):
    ...
    def __init__(
        self, num_of_class=None, use_centroid=True, use_cuda=True, noise_size=None
    ):
...
class SegNet_CNN4D_FC5C(nn.Module):
    ...
    def get_2d_patch_1_network(self):
        ...
        return nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=128, out_channels=32, kernel_size=5, stride=1, padding=0
            ),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=32, out_channels=8, kernel_size=5, stride=1, padding=0
            ),
            nn.BatchNorm2d(8),
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=8, out_channels=1, kernel_size=5, stride=1, padding=0
            ),
            nn.ReLU(inplace=True),
        )

    def get_2d_patch_3_network(self):
        # outputsize = 87 / 3 - 4 - 4 - 4 - 4 =13
        return nn.Sequential(
            nn.MaxPool2d(3),
            nn.Conv2d(
                in_channels=1, out_channels=128, kernel_size=5, stride=1, padding=0
    ...
    def get_3d_patch_network(self):
        ...
        return nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv3d(
                in_channels=64, out_channels=16, kernel_size=3, stride=1, padding=0
            ),
            nn.BatchNorm3d(16),
            nn.ReLU(inplace=True),
            nn.Conv3d(
                in_channels=16, out_channels=1, kernel_size=3, stride=1, padding=0
            ),
            nn.ReLU(inplace=True),
        )
...
class SegNet_CNN3D_FC7C1(nn.Module):
    ...
    def get_2d_patch_1_network(self):
        ...
        return nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=32, out_channels=1, kernel_size=5, stride=1, padding=0
            ),
            nn.ReLU(inplace=True),
        )
...
class SegNet_CNN21D_FC7C1(nn.Module):
    ...
    def __init__(
        ...
        if self.use_cuda:
            for i in range(len(self.cnn_layers)):
                self.cnn_layers[i] = self.cnn_layers[i].cuda()
            self.cnn_sharing_layer_patch_1_network = (
                self.cnn_sharing_layer_patch_1_network.cuda()
            )
            self.cnn_sharing_layer_patch_3_network = (
                self.cnn_sharing_layer_patch_3_network.cuda()
            )
            self.centroid_identity_layer = self.centroid_identity_layer.cuda()
            self.fully_connected_layer = self.fully_connected_layer.cuda()
    ...
    def forward(self, input_value):
        ...
        if self.use_cuda:
            cnn_input_list = [
                self.cnn_sharing_layer_patch_1_network(
                    input_value["patch_x_scale_1"].float().cuda()
                ),
                self.cnn_sharing_layer_patch_1_network(
                    input_value["patch_y_scale_1"].float().cuda()
                ),
                self.cnn_sharing_layer_patch_1_network(
                    input_value["patch_z_scale_1"].float().cuda()
                ),
                self.cnn_sharing_layer_patch_3_network(
                    input_value["patch_x_scale_3"].float().cuda()
                ),
                self.cnn_sharing_layer_patch_3_network(
                    input_value["patch_y_scale_3"].float().cuda()
                ),
                self.cnn_sharing_layer_patch_3_network(
                    input_value["patch_z_scale_3"].float().cuda()
                ),
...
class SegNetOnPaper(nn.Module):
    ...
    def __init__(
        ...
        super(SegNetOnPaper, self).__init__()

        if num_of_class is None:
            raise ValueError("No no_of_class")

        self.name = "SegNetOnPaper"
    ...
    def get_2d_patch_network(self):
        ...
        return nn.Sequential(
            nn.Conv2d(
                in_channels=32, out_channels=1, kernel_size=4, stride=1, padding=0
            ),
            nn.ReLU(inplace=True),
            nn.MaxPool2d(2),
        )
...
class SegNetOnPaperReplaceCNN(nn.Module):
    ...
    def __init__(
        ...
        self.cnn_sharing_layer_patch_1_network = nn.Sequential(
            ...
            nn.ReLU(inplace=True),
            nn.Conv2d(
                in_channels=32, out_channels=32, kernel_size=2, stride=2, padding=0
            ),
            nn.ReLU(inplace=True),
        )
