...
from loader.utils import (add_zero_padding_with_shape, get_min_max_value,
                          min_max_scaler, return_label_dicts,
                          return_largest_size)
...
def get_data(
    ...
    if data_file is None and data_dir:
        ...
        nii_files = [
            ...
            if file.endswith(".nii") and not file.endswith("glm.nii")
        ]
        glm_files = []
    ...
    data_list = [
        np.array(nib.load(data).dataobj).astype(np.uint16) for data in nii_files
    ]
    label_list = [
        np.array(nib.load(data).dataobj).astype(np.uint16) for data in glm_files
    ]
    ...
    for idx, single_data in enumerate(data_list):
        data[idx] = np.pad(
            add_zero_padding_with_shape(single_data, (x_s, y_s, z_s)),
...
class BrainSegmentationDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x["patch_x_scale_1"] = torch.unsqueeze(
            torch.from_numpy(sample[43][29:58, 29:58]), 0
        )
        x["patch_y_scale_1"] = torch.unsqueeze(
            torch.from_numpy(sample[:, 43][29:58, 29:58]), 0
        )
        x["patch_z_scale_1"] = torch.unsqueeze(
            torch.from_numpy(sample[:, :, 43][29:58, 29:58]), 0
        )
...
class BrainSegmentationDataset3D(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x["patch_3d"] = torch.unsqueeze(
            torch.from_numpy(sample[37:50, 37:50, 37:50]), 0
        )
